...
### Hashing tests
From the benchmarking results (see later) it can be seen that the different datasets (English sentences, English words, DNA sequences and random strings) seem to behave quite similar. There are only two differences noted: for the English sentences the hashing time seems to be longer as these are in general longer strings that need to be hashed. The second difference was the random strings: here the behavior was less stable as for the other datasets, this is probably because hashing tends to be more variable with the random sequence than the other, more structured, datasets.
...
- False positive rates
  ...
  - The observed false positive rate has also a one on one relationship with the theoretical false positive rate. This is the case when varying the false positive rate (exp_vs_obs_fpr_by_fpr.png). Even if the expected amount of elements to be inserted is varied (exp_vs_obs_fpr_by_item.png), the relationship stays nicely one on one.
...
- project_bloom: the actual project
  ...
  - docs: files containing the documentation of the project
  - results: benchmarking results and generated plots.
